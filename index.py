...
import telegram
import os
#import pygetwindow REMOVIDO, SÓ SERÁ IMPORTADO CASO O SISTEMA SEJA WINDOWS
...
############################ADICIONADAS AS FUNÇÕES DE CONTROLE DE JANELAS PARA LINUX#############################################################
...
def login():
    ...
    if clickbtn(images['connect-wallet'], timeout=15):
        ...
        time.sleep(3)
        if clickbtn(images['select-wallet-3'], timeout=15):
            time.sleep(5)
...
###############CÓDIGO ADICIONADO.  A VARIÁVEL 'bombcryptoWindows' TERÁ UM VALOR ATRIBUÍDO DEPENDENDO DO SISTEMA OPERACIONAL#########
...
####################################################################################################################################
    ...
    ##############FOR ALTERADO, O ORIGINAL ESTÁ COMENTADO ABAIXO##############################################################
        ...
        while True:
            for currentWindow in windows:
                ...
                if now - currentWindow['balance'] > addrandomness(t['get_balance'] * 60):
                    ...
                    curwind = get_linux_bombcrypto_windows
                    gobalance()
